...
export async function GET() {
  try {
    // Self-cleaning step: Auto-delete the static public/resume.pdf file if it exists.
    // This allows Next.js to fall back to our dynamic routes without manual user intervention.
    try {
      const staticPath = path.join(process.cwd(), 'public', 'resume.pdf');
      await fs.unlink(staticPath);
      console.log('[Routing Cleaned] Successfully removed static public/resume.pdf');
    } catch (cleanErr: any) {
      if (cleanErr.code !== 'ENOENT') {
        console.error('Failed to clean static file:', cleanErr);
      }
    }

    // 1. Define the actual formatted text stream for the PDF
    const streamBody = `BT
% Name
/F1 20 Tf
50 790 Td
...
`;
    ...
    // 2. Define the static PDF structure header with object catalog
...
`;
    ...
    // 3. Dynamically calculate exact byte offsets in memory
    let offset = 0;
    const offsets = [];

    offsets.push(pdfHeader.indexOf('1 0 obj'));
    offsets.push(pdfHeader.indexOf('2 0 obj'));
    offsets.push(pdfHeader.indexOf('3 0 obj'));
    offsets.push(pdfHeader.indexOf('4 0 obj'));
    offsets.push(pdfHeader.indexOf('5 0 obj'));

    offset += Buffer.byteLength(pdfHeader, 'utf-8');
    offsets.push(offset); // Obj 6 start offset

    offset += Buffer.byteLength(streamHeader, 'utf-8') + Buffer.byteLength(streamBody, 'utf-8') + Buffer.byteLength(streamFooter, 'utf-8');

    const startxref = offset;

    // Generate Cross-Reference (xref) table dynamically
    let xref = `xref
0 ${offsets.length + 1}
0000000000 65535 f \n`;
...
`;
    ...
    // 4. Assemble the exact PDF binary string
    const fullPdf = pdfHeader + streamHeader + streamBody + streamFooter + xref + trailer;

    // Return the response with proper PDF headers for direct rendering
    return new Response(Buffer.from(fullPdf, 'binary'), {
      headers: {
